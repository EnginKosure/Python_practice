...
def create_ReSQ_export(li, tn):
    ...
    return f"""
            ...
            SELECT * FROM `{tn}`
            ORDER BY COVERCODE_NEW, UNIQUE_ACCIDENT_YEAR, UNIQUE_DEVELOPMENT_PERIOD_IN_QUARTERS;
         """

# QUERY CREATOR-1-1
# extended version of 1, for manipulating the ordering for gross written premiums differently


def create_ReSQ_export_gross(li, tn):
    project = "geb-dwh-test"
    dataset_id = "uat_geb_dwh_eu_act"
    # source_table=tn
    # ctn=tn[39:]
    return f"""
            CREATE OR REPLACE TABLE `{project}.{dataset_id}.{li}` AS
            SELECT * FROM `{tn}`
            ORDER BY COVERCODE_NEW, AYEAR,AQUARTER;
         """
...
# TABLE_CHECKER (QUERY-3)
# Checks if tables are created before beginning the export phase


def reSQ_table_checker_query(tn, search_term, project, dataset_id):
    source_table = "INFORMATION_SCHEMA.TABLES"
    print("table name", tn)
    ...
    return f"""
           ...
           SELECT *
            FROM `{project}.{dataset_id}.{source_table}`
            WHERE table_name LIKE @search_term AND table_name NOT LIKE "source_%" ORDER BY table_name;
         """

# TABLE_CHECKER QUERY EXECUTER (Executes QUERY-3)


def reSQ_table_checker_query_executer(search_term, project, dataset_id, helper_tn):
    # create the export helper index table using info schema of bigquery
    tn = f'{project}.{dataset_id}.{helper_tn}'
    client_f = bigquery.Client()
    query_f = reSQ_table_checker_query(tn, search_term, project, dataset_id)
    ...
    # wait 3 sec. for helper index table creation so as to table creation function may have it when triggered
    sleep(3)
    table = client_f.get_table(tn)
    df = client_f.list_rows(table).to_dataframe()
    print(df)
    return df
...
def generate_tables(df, table_names):
    for j in range(0, len(df)):
        ...
        if "gross" in tn:
            query_ex = create_ReSQ_export_gross(li, tn)
...
def export_tables(df, table_names, dataset_ref, project, dataset_id):
    for j in range(0, len(df)):
        ...
        folder_name = 'resq_exports'
        table_id = table_names[j][39:]  # .lower()
        # print('table_id', table_id)

        destination_uri = f"gs://{bucket_name}/{folder_name}/{table_id}.csv"
...
def export_resq():
    ...
    checker_tn = "resq_created_table_names_for_export"
    checker_bool = True
    ...
    if confirm(table_names):
        ...
        while checker_bool:
            if len(reSQ_table_checker_query_executer(search_term, project, dataset_id, checker_tn)) == 5:
                print("EXPORT PHASE STARTED")
                # EXPORT TABLES

                export_tables(df, table_names, dataset_ref,
                              project, dataset_id)
                ...
                print('\nwait 10 sec. for table creation, then trigger extract\n')
                sleep(10)
